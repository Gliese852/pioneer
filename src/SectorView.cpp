...
// functor-callbacks for the SectorMap

class OnClickLabel {
	SectorView &sv;
public:
	OnClickLabel(SectorView &sv) : sv(sv) {}
	void operator()(const SystemPath &path)
	{
		sv.SwitchToPath(path);
	}
};

class SectorView::SystemDisplayMode {
	SectorView &sv;
public:
	SystemDisplayMode(SectorView &sv) : sv(sv) {}
	SectorMapContext::DisplayMode operator()(const SystemPath &path)
	{
		using MODES = SectorMapContext::DisplayModes;
		if (path.IsSameSystem(sv.m_selected)) return MODES::ALWAYS;
		if (path.IsSameSystem(sv.m_current)) return MODES::ALWAYS;
		if (std::find_if(sv.m_route.begin(), sv.m_route.end(), [path](const SystemPath &a) { return path.IsSameSystem(a); }) != sv.m_route.end())
			return MODES::ALWAYS;

		// in range
		RefCountedPtr<const Sector> sec1 = sv.m_game.GetGalaxy()->GetSector(path);
		RefCountedPtr<const Sector> sec2 = sv.m_game.GetGalaxy()->GetSector(sv.m_current);
		float dist = Sector::DistanceBetween(sec1, path.systemIndex, sec2, sv.m_current.systemIndex);
		if (dist > sv.m_playerHyperspaceRange && !sv.m_drawOutRangeLabels) return MODES::HIDE_LABEL;
...
SectorView::SectorView(Game *game) :
	...
	m_game(*game),
	m_map(new SectorMap({ game->GetGalaxy(), Pi::input, Pi::config, Pi::renderer, OnClickLabel(*this), SystemDisplayMode(*this) }))
{
	InitDefaults();
	...
	m_game(*game)
{
	...
	try {
		...
		m_map.reset(new SectorMap(sectorViewObj["map"], { game->GetGalaxy(), Pi::input, Pi::config, Pi::renderer, OnClickLabel(*this), SystemDisplayMode(*this) }));
