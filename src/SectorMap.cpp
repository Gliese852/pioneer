...
SectorMap::SectorMap(SectorMapContext &&context) :
	...
	m_pos{},
	m_posMovingTo{},
	m_context(new SectorMapContext(context)),
	m_labels(*this)
{
	InitDefaults();
...
{
	m_onMouseWheelCon =
		m_context->input->onMouseWheel.connect(sigc::mem_fun(this, &SectorMap::MouseWheel));
...
matrix4x4f SectorMap::PointOfView() {
	...
	result.Translate(0.f, 0.f, -10.f - 10.f * m_zoom);
...
void SectorMap::Draw3D()
{
	...
	if (m_zoomClamped <= FAR_THRESHOLD)
		renderer->SetPerspectiveProjection(40.f, renderer->GetDisplayAspect(), 1.f, 300.f);
	else
		renderer->SetPerspectiveProjection(40.f, renderer->GetDisplayAspect(), 1.f, 600.f);
...
void SectorMap::DrawLabels()
{
	...
	Pi::pigui->GetRenderer()->RenderDrawData(&drawData); //XXX
}
...
void SectorMap::OnClickLabel(const SystemPath &path)
{
	...
	m_context->onClickLabel(path);
}

void SectorMap::PutSystemLabel(const Sector::System &sys)
{
	PROFILE_SCOPED()
	...
	if (screenPos.z < 0.0f && (sys.GetPopulation() > 0 || m_drawUninhabitedLabels)) {
		...
		// work out the colour
		Color labelColor = sys.GetFaction()->AdjustedColour(sys.GetPopulation(), true); // XXX
...
void SectorMap::DrawNearSector(const int sx, const int sy, const int sz, const matrix4x4f &trans)
{
	...
	for (std::vector<Sector::System>::iterator i = ps->m_systems.begin(); i != ps->m_systems.end(); ++i, ++sysIdx) {
		...
		auto showMode = m_context->systemDisplayMode(i->GetPath());
		...
		// add label
		if (!(showMode & m_context->HIDE_LABEL)) PutSystemLabel(*i);
	}
}
...
void SectorMap::BuildFarSector(RefCountedPtr<Sector> sec, const vector3f &origin, std::vector<vector3f> &points, std::vector<Color> &colors)
{
	...
	for (std::vector<Sector::System>::iterator i = sec->m_systems.begin(); i != sec->m_systems.end(); ++i) {
		...
		m_visibleFactions.insert(i->GetFaction());
		if (m_hiddenFactions.find(i->GetFaction()) != m_hiddenFactions.end()) continue; // XXX
...
void SectorMap::MouseWheel(bool up)
{
	//if (this == Pi::GetView()) { // XXX
		if (!up)
