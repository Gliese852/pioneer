...
-- sorted table, equipment -> slot
ShipEquip.table = {}

-- TODO implement? crashes without it
...
function ShipEquip:fromStartVariant(variant)
	...
	self.value = {}
	local stack = {{ slots = variant.equipment }}

	-- flat list of all equipment
	while #stack > 0 do
		local item = table.remove(stack)
		for slot, eq in pairs(item.slots) do
			if type(eq) == 'string' then
				table.insert(self.value, { slot = slot, id = eq, parent = item.parent })
			elseif type(eq) == 'table' then
				assert(eq.id)
				assert(eq.slots)
				assert(type(eq.slots) == 'table')
				table.insert(self.value, { slot = slot, id = eq.id, parent = item.parent })
				table.insert(stack, { parent = { slot = slot, parent = item.parent }, slots = eq.slots })
			else
				assert(false, "invalid equipment structure")
			end
		end
	end
	...
	function slotLess(x1, x2)
		if type(x1) ~= type(x2) then return type(x1) == 'string'
		else return x1 < x2
		end
	end
	function slotEqual(x1, x2)
		return type(x1) == type(x2) and x1 == x2
	end

	-- child items always follow their parent
	table.sort(self.value, function(x1, x2)
		if x1.parent == x2.parent then
			return slotLess(x1.slot, x2.slot)
		end
		local p1 = parents(x1)
		local p2 = parents(x2)

		while p1 and p2 do
			if not slotEqual(p1.slot, p2.slot) then return slotLess(p1.slot, p2.slot) end
			p1 = p1.child
			p2 = p2.child
		end

		if p1 then return false
		elseif p2 then return true
		else assert(false, "probably identical slot structure for both elements")
		end
	end)
end
